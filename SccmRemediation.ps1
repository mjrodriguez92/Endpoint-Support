<#/
This script will perform a clean uninstall of SCCM from an endpoint. This process is necessary
to relinquish the MDM authority SCCM has over a co-managed device to allow Intune to claim
sole MDM authority. Following execution the device will re-evaluate and re-sync to intune and
within 30 minutes re-evaluate and reflect as Intune managed in the Intune cloud console.
/#>
...
# Cleanup all traces of SCCM from the computer and trigger a re-sync to establish new MDM authority
function removeSCCM() {
    # Stop SCCM services
    Get-Service -Name CcmExec -ErrorAction SilentlyContinue | Stop-Service -Force -Verbose
    Get-Service -Name ccmsetup -ErrorAction SilentlyContinue | Stop-Service -Force -Verbose
...
# Runs a sync on the device to Intune using the Intune management extension service. This will have Intune sync to take MDM authority
$newShell = New-Object -ComObject Shell.Application
$newShell.open("intunemanagementextension://synccompliance")